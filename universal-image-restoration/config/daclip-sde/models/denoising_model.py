...
class DenoisingModel(BaseModel):
    ...
    def test(self, sde=None, mode='posterior', num_steps=1000, save_states=False):
        sde.set_mu(self.condition)

        self.model.eval()
        with torch.no_grad():
            if mode == 'sde':
                self.output = sde.reverse_sde(self.state, num_steps=num_steps, save_states=save_states, text_context=self.text_context, image_context=self.image_context)
            else:
                self.output = sde.reverse_posterior(self.state, num_steps=num_steps, save_states=save_states, text_context=self.text_context, image_context=self.image_context)
