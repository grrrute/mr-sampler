...
class IRSDE(SDE):
    ...
    def _initialize(self, max_sigma, T, schedule, eps=0.01):
        ...
        thetas_cumsum = get_thetas_cumsum(thetas)
    ...
    def reverse_sde(self, xt, num_steps=-1, save_states=False, **kwargs):
        N = self.sample_T if num_steps < 0 else num_steps
        timesteps = np.linspace(1, self.sample_T, N, dtype=float).tolist()

        x = xt.clone()
        x_list = [x.cpu()]
        for t in tqdm(reversed(timesteps), total=N):
            score = self.score_fn(x, int(t), self.sample_scale, **kwargs)
            x = self.reverse_sde_step(x, score, int(t))       
    ...
    def reverse_posterior(self, xt, num_steps=-1, save_states=False, save_dir='posterior_state', **kwargs):
        N = self.sample_T if num_steps < 0 else num_steps
        timesteps = np.linspace(1, self.sample_T, N, dtype=float).tolist()
        x = xt.clone()
        x_list = [x.cpu()]
        for t in tqdm(reversed(timesteps), total=N):
            noise = self.noise_fn(x, int(t), self.sample_scale, **kwargs)
            x = self.reverse_posterior_step(x, noise, int(t))            
    ...
    def ode_sampler(self, xt, rtol=1e-5, atol=1e-5, method='RK45', eps=1e-3, text_context=None, image_context=None):
        shape = xt.shape
        ...
        def ode_func(t, x):
            ...
            epsilon = self.noise_fn(x, t, text_context=text_context, image_context=image_context)
            score = -epsilon / self.sigmas[t]
        ...
        x0 = torch.tensor(solution.y[:, -1]).reshape(shape).to(self.device).type(torch.float32)
        states = [xt.cpu()]
        for i in range(len(solution.t)):
            states.append(from_flattened_numpy(solution.y[:, i], shape).cpu())
        nfe = solution.nfev
        return x0, states, nfe
